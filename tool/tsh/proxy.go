...
func onProxyCommandSSH(cf *CLIConf) error {
	...
	proxyParams, err := makeSSHProxyParams(cf, tc)
	if err != nil {
		return trace.Wrap(err)
	}
...
// makeSSHProxyParams prepares parameters for establishing an SSH proxy.
func makeSSHProxyParams(cf *CLIConf, tc *libclient.TeleportClient) (*sshProxyParams, error) {
	...
	sshProxyHost, sshProxyPort, err := sshProxyHostPort(*ping)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	err = tc.WithoutJumpHosts(func(tcNoJump *libclient.TeleportClient) error {
		return tc.LoadKeyForClusterWithReissue(cf.Context, ping.ClusterName)
	})
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func sshProxyHostPort(pr webclient.PingResponse) (string, string, error) {
	publicAddr, err := utils.ParseAddr(pr.Proxy.SSH.PublicAddr)
	if err != nil {
		return "", "", trace.Wrap(err)
	}
	if pr.Proxy.TLSRoutingEnabled {
		return publicAddr.Host(), strconv.Itoa(publicAddr.Port(defaults.HTTPListenPort)), nil
	}
	sshListenAddr, err := utils.ParseAddr(pr.Proxy.SSH.ListenAddr)
	if err != nil {
		return "", "", trace.Wrap(err)
	}
	if pr.Proxy.SSH.SSHPublicAddr != "" {
		sshPublicAddr, err := utils.ParseAddr(pr.Proxy.SSH.SSHPublicAddr)
		if err != nil {
			return "", "", trace.Wrap(err)
		}
		return sshPublicAddr.Host(), strconv.Itoa(sshPublicAddr.Port(sshListenAddr.Port(defaults.SSHProxyListenPort))), nil
	}
	return publicAddr.Host(), strconv.Itoa(sshListenAddr.Port(defaults.SSHProxyListenPort)), nil
}
