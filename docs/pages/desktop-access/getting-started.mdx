...
In this step we create a certificate template that uses elliptic curve P-384 and uses SHA384 as the signature algorithm.
...
6. Go to `General` tab and change `Template display name` to `RemoteDesktopAccess`. Make sure `Template name` is also `RemoteDesktopAccess`.
...
8. Next, in the `Extensions` tab select `Application Polices` and click `Edit` button.
9. Remove all entries from the list.
10. Go to `Security` tab, select `Domain Controllers` and give the group `Read` and `Enroll` permissions.
11. Finally, create template by clicking `OK`
