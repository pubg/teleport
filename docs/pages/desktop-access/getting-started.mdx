...
In this step we will create a certificate template that is using elliptic curve P-384 and is using SHA384 as an signature algorithim.
...
9. Go to `Security` tab, select `Domain Controllers` and give the group `Read` and `Enroll` permissions.
10. Finally, create template by clicking `OK`
