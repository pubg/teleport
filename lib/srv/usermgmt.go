...
func (u *userCloser) Close() error {
	...
	u.usersSync.Lock()
	defer u.usersSync.Unlock()
	err = u.users.DeleteUser(u.username, teleportGroup.Gid)
	delete(u.presentUsers, u.username)
	return trace.Wrap(err)
}
...
func (u *HostUserManagment) CreateUser(name string, ui *services.HostUsersInfo) (io.Closer, error) {
	...
	if tempUser != nil {
		...
		return &userCloser{
			...
			users:        u,
			backend:      u.backend,
			presentUsers: u.presentUsers,
			usersSync:    u.usersSync,
...
func (u *HostUserManagment) DeleteAllUsers() error {
	...
	for _, name := range users {
		if _, ok := u.presentUsers[name]; ok {
